            ...
            <v-card class="mx-auto"  :max-width="$vuetify.breakpoint.lgAndDown ? 900 : 1200">
                ...
                <div id="filters" class="ma-2">
                    <v-row align-content ="center">
                        ...
                        <v-col  offset ="2" offset-sm="0"  class="mt-2" cols="8" sm="2">
                            <v-menu ref="menu" v-model="menu" :close-on-content-click="false"
                                ...
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field class="calendar" v-model="date" label="Since" prepend-icon="mdi-calendar"
                        ...
                        <v-spacer></v-spacer>
                        <v-col offset ="2" offset-sm="0"  class="mt-2" cols="8" sm="2">
                            <v-text-field v-model="filterLength" label="Tweets viewed" placeholder="x" type="number" min="0" ></v-text-field>
                        </v-col>
...
export default {
    ...
    methods: {
        async getReplacement() {
            ...
            while (this.tweets.find(x => x._id == replacement._id)) {
                i = i + 1
                replacement = (await AnnotationService.getOne(this.$store.state.auth.user._id, i,
                    {"date" : this.date, 
                    "categories" : this.categories})).data
            }
            if (replacement) {
        ...
        async applyFilters () {
            this.tweets =  (await AnnotationService.getFiltered ({"user" :this.$store.state.auth.user._id, 
                                                            "date" : this.date, 
